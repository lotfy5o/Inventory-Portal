...
--
-- Host: 127.0.0.1
-- Generation Time: Oct 06, 2023 at 11:25 PM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `st_getCustomer` ()   BEGIN 
	SELECT * FROM customer ORDER BY customer.cust_name asc;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `st_insertCustomerLedger` (`dt` DATE, `cuid` INT, `descr` VARCHAR(100), `debit` FLOAT, `credit` FLOAT, `balance` FLOAT)   BEGIN 
	INSERT into ledgercustomer (ld_date, ld_cuid, ld_desc, ld_debit, ld_credit, ld_balance)
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `st_insertSupplierLedger` (`dt` DATE, `sid` INT, `descr` VARCHAR(100), `debit` FLOAT, `credit` FLOAT, `balance` FLOAT)   BEGIN 
	INSERT into ledgersupplier (ld_date, ld_sid, ld_desc, ld_debit, ld_credit, ld_balance)
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `st_updateSizes` (IN `name` VARCHAR(20), IN `id` INT, IN `catID` INT)   BEGIN
	UPDATE sizes
...
CREATE TABLE `ledgercustomer` (
  ...
  `ld_cuid` bigint(20) NOT NULL,
  `ld_desc` varchar(100) NOT NULL,
  `ld_debit` float NOT NULL DEFAULT 0,
  `ld_credit` float NOT NULL DEFAULT 0,
  `ld_balance` float NOT NULL DEFAULT 0
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
--
ALTER TABLE `ledgercustomer`
  MODIFY `ld_id` bigint(20) NOT NULL AUTO_INCREMENT;
...
--
ALTER TABLE `sizes`
  MODIFY `si_id` tinyint(4) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=7;
...
--
ALTER TABLE `ledgercustomer`
  ADD CONSTRAINT `ledgercustomer_ibfk_1` FOREIGN KEY (`ld_cuid`) REFERENCES `customer` (`cust_id`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Constraints for table `ledgersupplier`
--
ALTER TABLE `ledgersupplier`
  ADD CONSTRAINT `ledgersupplier_ibfk_1` FOREIGN KEY (`ld_sid`) REFERENCES `suppliers` (`supp_id`) ON DELETE CASCADE ON UPDATE CASCADE;
