...
--
-- Host: 127.0.0.1
-- Generation Time: Nov 03, 2023 at 08:48 PM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `st_deleteOwner` (`id` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `st_getLastOwnerID` ()   BEGIN
	SELECT owner.o_id as 'ID' FROM owner ORDER BY owner.o_name DESC LIMIT 1;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `st_getOwners` ()   BEGIN
	SELECT 
        o.o_id AS "ID", 
        o.o_name AS "Owner", 
        o.o_phone AS "Phone", 
        o.o_address AS "Address",
        ledgerowner.lo_balance AS "Balance"
    FROM owner o
    INNER JOIN ledgerowner
    on o.o_id = ledgerowner.lo_oid
    WHERE ledgerowner.lo_desc = "Opening Balance"
    ORDER BY o.o_name ASC;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `st_insertOwner` (`name` VARCHAR(100), `phone` VARCHAR(15), `addr` VARCHAR(100))   BEGIN

	INSERT INTO owner (o_name, o_phone, o_address) VALUES (name, phone, addr);

END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `st_insertOwnerLedger` (IN `dt` DATE, IN `oid` INT, IN `descr` VARCHAR(100), IN `debit` FLOAT, IN `credit` FLOAT, IN `balance` FLOAT)   BEGIN 
	INSERT into ledgerowner (lo_date, lo_oid, lo_desc, lo_debit, lo_credit, lo_balance)
    VALUES (dt, oid, descr, debit, credit, balance);
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `st_insertProduct` (IN `proName` VARCHAR(50))   BEGIN
	INSERT INTO products (pro_name) VALUES (proName);
        

END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `st_insertProductPrice` (IN `proID` BIGINT, IN `price` FLOAT, IN `catID` INT, IN `sizeID` TINYINT, IN `colID` INT)   BEGIN
	INSERT into productprice (pp_proID, pp_price, pp_catID, pp_sizeID, pp_colID) VALUES (proID, price, catID, sizeID, colID);
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `st_insertPurchaseInvoiceDetails` (IN `purID` BIGINT, IN `proID` BIGINT, IN `ppp` FLOAT, IN `quan` INT, IN `catID` INT, IN `sizeID` TINYINT, IN `colID` INT)   BEGIN
	INSERT INTO purchaseinvoicedetails (pid_purID, pid_proID, pid_perPiecePrice, pid_quantity, pid_catID, pid_sizeID, pid_colID) 
    VALUES (purID, proID, ppp, quan, catID, sizeID, colID);
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `st_insertStock` (IN `proID` BIGINT, IN `quan` INT, IN `purID` BIGINT, IN `catID` INT, IN `sizeID` TINYINT, IN `colID` INT)   BEGIN
	INSERT into stock (sto_proID, sto_quan, sto_purID, sto_catID, sto_sizeID, sto_colID) VALUES (proID, quan, purID, catID, sizeID, colID);
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `st_updateOwner` (`name` VARCHAR(100), `phone` VARCHAR(15), `addr` VARCHAR(100), `id` INT)   BEGIN

	UPDATE owner
    SET owner.o_name = name,
    	owner.o_phone = phone, 
        owner.o_address = addr
        
    WHERE owner.o_id = id;

END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `st_updateProduct` (IN `proName` VARCHAR(50), IN `proID` BIGINT)   BEGIN
	UPDATE products
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `st_updatePurchaseInvoiceDetails` (IN `purID` BIGINT, IN `proID` BIGINT, IN `ppp` FLOAT, IN `quan` INT, IN `pidID` INT, IN `catID` INT, IN `sizeID` TINYINT, IN `colID` INT)   BEGIN
...
--
-- Table structure for table `ledgerowner`
--

CREATE TABLE `ledgerowner` (
  `lo_id` bigint(20) NOT NULL,
  `lo_oid` int(11) NOT NULL,
  `lo_date` date NOT NULL,
  `lo_desc` varchar(100) NOT NULL,
  `lo_debit` float NOT NULL,
  `lo_credit` float NOT NULL,
  `lo_balance` float NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Dumping data for table `ledgerowner`
--

INSERT INTO `ledgerowner` (`lo_id`, `lo_oid`, `lo_date`, `lo_desc`, `lo_debit`, `lo_credit`, `lo_balance`) VALUES
(1, 3, '2023-11-03', 'Opening Balance', 1000, 0, 1000);
...
INSERT INTO `owner` (`o_id`, `o_name`, `o_phone`, `o_address`) VALUES
(2, 'Ahmed', '111111111', 'district 29'),
(3, 'lotfy', '0101111111', '29dis');
...
CREATE TABLE `productprice` (
  ...
  `pp_catID` int(11) DEFAULT NULL,
  `pp_sizeID` tinyint(4) DEFAULT NULL,
  `pp_colID` int(11) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
INSERT INTO `products` (`pro_id`, `pro_name`) VALUES
(1, 'Denim');
...
INSERT INTO `purchaseinvoice` (`pur_id`, `pur_date`, `pur_supplierID`) VALUES
(14, '2023-10-18', 14);
...
CREATE TABLE `stock` (
  ...
  `sto_catID` int(11) DEFAULT NULL,
  `sto_sizeID` tinyint(4) DEFAULT NULL,
  `sto_colID` int(11) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
(17, 'MariamLove', '1234555555', 'district 292');
...
--
-- Indexes for table `ledgerowner`
--
ALTER TABLE `ledgerowner`
  ADD PRIMARY KEY (`lo_id`),
  ADD KEY `lo_oid` (`lo_oid`);
...
ALTER TABLE `productprice`
  ...
  ADD UNIQUE KEY `uni_pp` (`pp_proID`,`pp_catID`,`pp_sizeID`,`pp_colID`),
  ADD KEY `pp_catID` (`pp_catID`),
  ADD KEY `pp_sizeID` (`pp_sizeID`),
  ADD KEY `pp_colID` (`pp_colID`);
...
ALTER TABLE `stock`
  ...
  ADD UNIQUE KEY `uni_stock` (`sto_proID`,`sto_purID`,`sto_catID`,`sto_sizeID`,`sto_colID`),
  ADD KEY `sto_purID` (`sto_purID`),
  ADD KEY `sto_catID` (`sto_catID`),
  ADD KEY `sto_sizeID` (`sto_sizeID`),
  ADD KEY `sto_colID` (`sto_colID`);
...
--
-- AUTO_INCREMENT for table `ledgerowner`
--
ALTER TABLE `ledgerowner`
  MODIFY `lo_id` bigint(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3;
...
--
ALTER TABLE `productprice`
  MODIFY `pp_id` bigint(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=5;
...
--
ALTER TABLE `purchaseinvoice`
  MODIFY `pur_id` bigint(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=15;

--
-- AUTO_INCREMENT for table `purchaseinvoicedetails`
--
ALTER TABLE `purchaseinvoicedetails`
  MODIFY `pid_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=17;
...
--
ALTER TABLE `sizes`
  MODIFY `si_id` tinyint(4) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=27;

--
-- AUTO_INCREMENT for table `stock`
--
ALTER TABLE `stock`
  MODIFY `sto_id` bigint(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=8;
...
--
-- Constraints for table `ledgerowner`
--
ALTER TABLE `ledgerowner`
  ADD CONSTRAINT `ledgerowner_ibfk_1` FOREIGN KEY (`lo_oid`) REFERENCES `owner` (`o_id`) ON DELETE CASCADE ON UPDATE CASCADE;
...
ALTER TABLE `productprice`
  ...
  ADD CONSTRAINT `productprice_ibfk_2` FOREIGN KEY (`pp_catID`) REFERENCES `categories` (`cat_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `productprice_ibfk_3` FOREIGN KEY (`pp_sizeID`) REFERENCES `sizes` (`si_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `productprice_ibfk_4` FOREIGN KEY (`pp_colID`) REFERENCES `colors` (`col_id`) ON DELETE CASCADE ON UPDATE CASCADE;
...
ALTER TABLE `stock`
  ...
  ADD CONSTRAINT `stock_ibfk_3` FOREIGN KEY (`sto_catID`) REFERENCES `categories` (`cat_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `stock_ibfk_4` FOREIGN KEY (`sto_sizeID`) REFERENCES `sizes` (`si_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `stock_ibfk_5` FOREIGN KEY (`sto_colID`) REFERENCES `colors` (`col_id`) ON DELETE CASCADE ON UPDATE CASCADE;
